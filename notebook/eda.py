...
def calculate_win_proba(df: pd.DataFrame) -> pd.DataFrame:
    df_new = df.copy(deep=True)
    df_new["WScore_mod"] = df_new["ScoreA"] // 2
    df_new["LScore_mod"] = df_new["ScoreB"] // 2
    df_new["AllScore_mod"] = df_new["WScore_mod"] + df_new["LScore_mod"]
    df_new["WinScoreProbEstimate"] = df_new["WScore_mod"] / (
        df_new["WScore_mod"] + df_new["LScore_mod"]
    )
    df_new["WinProb"] = df_new.apply(
        lambda x: 1
        - binom.cdf(
            x["AllScore_mod"] // 2, x["AllScore_mod"], x["WinScoreProbEstimate"]
        ),
        axis=1,
    )
    df_new.drop(
        ["WScore_mod", "LScore_mod", "AllScore_mod", "WinScoreProbEstimate"],
        inplace=True,
        axis=1,
    )
    return df_new
